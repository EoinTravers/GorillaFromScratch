...
let state = {
    ...
    big_font: null, // Between-subject manipulation
    trial_nr: 0,
...
function Ready(){
    ...
    let big_font = state.big_font = primate.manipulation('big_font', flip());
    let font_size = globals.font_sizes[big_font] + 'em';
...
function PrepareTrial(){
    ...
    let inner = state.target_right ? '>' : '<';
    let outer = state.flanker_right ? '>' : '<';
    let stim = _.repeat(outer, 2) + inner + _.repeat(inner, 2);
    $('#target').text(stim);
    $('#fix').show();
...
function ProcessResponse(k){
    ...
    let accuracy = Number(state.target_right == said_right);
    if(accuracy){
        LogData();
    } else {
        ErrorFeedback();
